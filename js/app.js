...
const DECOR=[
  {id:'rug',name:'Cozy rug',cost:40,set:'home',pos:'left:50%;bottom:5%;transform:translateX(-50%);width:64%;z-index:2',svg:'<svg viewBox="0 0 200 56" width="100%"><ellipse cx="100" cy="30" rx="96" ry="24" fill="#F2C2D6"/><ellipse cx="100" cy="30" rx="70" ry="16" fill="#FBE3EC"/><ellipse cx="100" cy="30" rx="44" ry="9" fill="#F7CFDE"/></svg>'},
  {id:'plant',name:'Leafy plant',cost:70,set:'home',pos:'left:5%;bottom:30%;width:20%;z-index:3',svg:'<svg viewBox="0 0 60 96" width="100%"><path d="M30 56C14 50 12 30 16 16c10 4 16 22 14 40z" fill="#8FD7BC"/><path d="M30 56C46 48 50 28 46 14c-12 4-18 24-16 42z" fill="#A6E0C9"/><path d="M30 60c-2-16 0-30 0-30s2 14 0 30z" fill="#5FB894"/><path d="M20 60h20l-3 30H23z" fill="#E7A6BE"/></svg>'},
  {id:'cushion',name:'Floor cushion',cost:35,set:'home',pos:'right:10%;bottom:8%;width:22%;z-index:4',svg:'<svg viewBox="0 0 70 52" width="100%"><rect x="4" y="10" width="62" height="36" rx="14" fill="#C7B4EC"/><rect x="4" y="10" width="62" height="36" rx="14" fill="none" stroke="#A98EE0" stroke-width="2"/><circle cx="35" cy="28" r="4" fill="#A98EE0"/></svg>'},
  {id:'lamp',name:'Warm lamp',cost:90,set:'home',pos:'right:4%;bottom:30%;width:18%;z-index:3',svg:'<svg viewBox="0 0 50 110" width="100%"><path d="M10 40h30l-6-22H16z" fill="#F6D27A"/><rect x="23" y="40" width="4" height="56" fill="#C9A98A"/><ellipse cx="25" cy="100" rx="16" ry="6" fill="#C9A98A"/></svg>'},
  {id:'frame',name:'Photo frame',cost:60,set:'home',pos:'left:9%;top:14%;width:18%;z-index:3',svg:'<svg viewBox="0 0 60 54" width="100%"><rect x="4" y="4" width="52" height="42" rx="6" fill="#fff" stroke="#E7C9B0" stroke-width="3"/><circle cx="30" cy="22" r="8" fill="#F2C2D6"/><path d="M14 40c4-9 12-9 16 0M30 40c3-6 9-6 12 0" fill="#9FD9BE"/></svg>'},
  {id:'flowers',name:'Flower vase',cost:50,set:'home',pos:'left:32%;bottom:7%;width:13%;z-index:4',svg:'<svg viewBox="0 0 44 70" width="100%"><circle cx="14" cy="16" r="7" fill="#F2A0BC"/><circle cx="30" cy="13" r="7" fill="#C7B4EC"/><circle cx="22" cy="22" r="7" fill="#F6D27A"/><path d="M22 26v18" stroke="#7FBE9C" fill="none" stroke-width="2"/><path d="M12 44h20l-3 22H15z" fill="#9FD0E8"/></svg>'}
];
// café treats — consumable, raise Bond + collection
const TREATS=[
  {id:'cupcake',name:'Cupcake',cost:25,bond:5,set:'treats',svg:'<svg viewBox="0 0 40 50" width="100%"><path d="M8 22h24l-3 24H11z" fill="#F4D9A6"/><path d="M8 23c0-7 5-12 12-12s12 5 12 12z" fill="#F7B6CE"/><circle cx="20" cy="9" r="3" fill="#E2607F"/></svg>'},
  {id:'smoothie',name:'Berry smoothie',cost:22,bond:5,set:'treats',svg:'<svg viewBox="0 0 30 52" width="100%"><path d="M7 15h16l-2 32H9z" fill="#F2A0BC"/><rect x="6" y="11" width="18" height="5" rx="2" fill="#fff"/><rect x="18" y="2" width="3" height="13" fill="#9FD9BE"/></svg>'},
  {id:'icecream',name:'Ice cream',cost:28,bond:6,set:'treats',svg:'<svg viewBox="0 0 28 56" width="100%"><path d="M9 26l5 28 5-28z" fill="#F4D9A6"/><circle cx="14" cy="18" r="9" fill="#C7B4EC"/><circle cx="14" cy="11" r="7" fill="#F7B6CE"/></svg>'},
  {id:'fruitbowl',name:'Fruit bowl',cost:30,bond:6,set:'treats',svg:'<svg viewBox="0 0 50 38" width="100%"><circle cx="16" cy="15" r="8" fill="#F2A0BC"/><circle cx="32" cy="13" r="8" fill="#9FD9BE"/><circle cx="25" cy="19" r="8" fill="#F6D27A"/><path d="M6 19h38l-4 15H10z" fill="#E7C9B0"/></svg>'},
  {id:'pretzel',name:'Soft pretzel',cost:26,bond:5,set:'treats',svg:'<svg viewBox="0 0 44 44" width="100%"><path d="M10 30c-6-6-2-16 6-16s14 8 8 14-14 6-18-2c-3-6 2-12 8-12" fill="none" stroke="#D9A05B" stroke-width="5" stroke-linecap="round"/></svg>'}
];
// accessories — worn by Lila. shop: boutique | spa
const ACC=[
  {id:'bow',name:'Pink bow',cost:40,set:'wardrobe',shop:'boutique',pos:'top:0%;left:50%;transform:translateX(-50%);width:30%',svg:'<svg viewBox="0 0 40 24" width="100%"><path d="M20 12c-4-9-16-7-16 0s12 9 16 0z" fill="#F2A0BC"/><path d="M20 12c4-9 16-7 16 0s-12 9-16 0z" fill="#F2A0BC"/><circle cx="20" cy="12" r="4" fill="#E2607F"/></svg>'},
  {id:'crown',name:'Flower crown',cost:90,set:'wardrobe',shop:'boutique',pos:'top:-4%;left:50%;transform:translateX(-50%);width:48%',svg:'<svg viewBox="0 0 84 26" width="100%"><path d="M6 22q36 -16 72 0" stroke="#8FD7BC" fill="none" stroke-width="3"/>'+[[16,'#F2A0BC'],[33,'#F6D27A'],[51,'#C7B4EC'],[68,'#F2A0BC']].map(([x,c])=>`<circle cx="${x}" cy="11" r="6.5" fill="${c}"/><circle cx="${x}" cy="11" r="2.5" fill="#fff"/>`).join('')+'</svg>'},
  {id:'clip',name:'Star clip',cost:35,set:'wardrobe',shop:'boutique',pos:'top:6%;left:20%;width:18%',svg:'<svg viewBox="0 0 24 24" width="100%"><path d="M12 2l3 6 6 1-4.5 4.5 1 6L12 17l-5.5 2.5 1-6L4 9l6-1z" fill="#F6D27A"/></svg>'},
  {id:'shades',name:'Heart shades',cost:70,set:'wardrobe',shop:'boutique',pos:'top:22%;left:50%;transform:translateX(-50%);width:44%',svg:'<svg viewBox="0 0 60 24" width="100%"><path d="M26 7h8" stroke="#E2607F" stroke-width="3"/><path d="M15 6c-5-3-11 2-7 7 2 3 7 6 7 6s5-3 7-6c4-5-2-10-7-7z" fill="#E2607F"/><path d="M45 6c-5-3-11 2-7 7 2 3 7 6 7 6s5-3 7-6c4-5-2-10-7-7z" fill="#E2607F"/></svg>'},
  {id:'party',name:'Party hat',cost:60,set:'wardrobe',shop:'boutique',pos:'top:-22%;left:52%;transform:translateX(-50%);width:26%',svg:'<svg viewBox="0 0 30 40" width="100%"><path d="M15 2l10 32H5z" fill="#C7B4EC"/><circle cx="15" cy="3" r="3" fill="#F6D27A"/></svg>'},
  ...
  {id:'sunhat',name:'Sun hat',cost:80,set:'wardrobe',shop:'spa',pos:'top:-8%;left:50%;transform:translateX(-50%);width:54%',svg:'<svg viewBox="0 0 80 30" width="100%"><ellipse cx="40" cy="22" rx="38" ry="7" fill="#F6D27A"/><path d="M22 22c0-12 8-18 18-18s18 6 18 18z" fill="#FBE3A0"/><path d="M22 22h36" stroke="#E2607F" stroke-width="3"/></svg>'},
  {id:'scarf',name:'Cozy scarf',cost:75,set:'wardrobe',shop:'spa',pos:'top:46%;left:50%;transform:translateX(-50%);width:46%',svg:'<svg viewBox="0 0 60 30" width="100%"><path d="M6 6q24 14 48 0v6q-24 14-48 0z" fill="#9FD9BE"/><path d="M44 12l4 16h-8z" fill="#7FBE9C"/></svg>'}
];
// garden seeds → blooms (permanent in park)
const SEEDS=[
  {id:'tulip',name:'Tulip',cost:30,set:'garden',bloom:'<svg viewBox="0 0 40 50" width="100%"><path d="M10 50V24M20 50V20M30 50V24" stroke="#5FB894" stroke-width="3"/><path d="M10 24c-4 0-6-4-4-8 2 2 4 2 4 2s2 0 4-2c2 4 0 8-4 8z" fill="#F2A0BC"/><path d="M20 20c-4 0-6-4-4-8 2 2 4 2 4 2s2 0 4-2c2 4 0 8-4 8z" fill="#E2607F"/><path d="M30 24c-4 0-6-4-4-8 2 2 4 2 4 2s2 0 4-2c2 4 0 8-4 8z" fill="#C7B4EC"/></svg>'},
  {id:'sunflower',name:'Sunflower',cost:35,set:'garden',bloom:'<svg viewBox="0 0 44 60" width="100%"><path d="M22 60V28" stroke="#5FB894" stroke-width="3"/>'+[0,45,90,135,180,225,270,315].map(a=>`<ellipse cx="22" cy="16" rx="4" ry="9" fill="#F6D27A" transform="rotate(${a} 22 16)"/>`).join('')+'<circle cx="22" cy="16" r="7" fill="#C77E12"/></svg>'},
  {id:'rose',name:'Rose',cost:40,set:'garden',bloom:'<svg viewBox="0 0 40 54" width="100%"><path d="M20 54V28" stroke="#5FB894" stroke-width="3"/><circle cx="20" cy="18" r="11" fill="#E2607F"/><circle cx="20" cy="18" r="6" fill="#F2A0BC"/><circle cx="20" cy="18" r="2.5" fill="#fff"/></svg>'}
];
const SETS={
  treats:{name:'Sweet treats',items:TREATS},
  wardrobe:{name:"Lila's wardrobe",items:ACC},
  home:{name:'Home comforts',items:DECOR},
  garden:{name:'Garden blooms',items:SEEDS}
};
const BADGES=[
  ...
  {id:'bond5',name:'Best friends',icon:'heart',color:'pink',test:s=>bondInfo(s.bond.xp).level>=5},
  ...
  {id:'spa',name:'Spa unlocked',icon:'sparkles2',color:'mint',test:s=>s.places.includes('spa')},
  {id:'s30',name:'A whole month',icon:'trophy',color:'amber',test:s=>s.streak.best>=30},
  {id:'green',name:'Green thumb',icon:'sprout',color:'green',test:s=>s.gardenHarvested.length>=3},
  {id:'collector',name:'Collector',icon:'gift',color:'lav',test:s=>['treats','wardrobe','home','garden'].some(k=>setDone(k,s))}
];

const PERFECT_BONUS=15, SPA_COST_HEARTS=8, STREAK_HEART_MILES=[3,7,14,30,60,100];
const LILA_NOTES=['I love spending the day with you 🤍','You make me so happy!','Best friends forever?','I believe in you, always.','Today felt lovely with you.','Thank you for taking care of me!'];
...
function relDay(ds){const n=daysBetween(ds,today());return n===0?'today':n===1?'yesterday':n+' days ago';}

/* ===== state ===== */
const KEY='lila.v1';
let state;
function freshState(){return{v:3,name:'friend',createdAt:today(),sparks:0,lifetime:0,hearts:0,
...
/* ===== rewards ===== */
function addSparks(n){const b=levelInfo(state.lifetime).level;state.sparks+=n;state.lifetime+=n;const a=levelInfo(state.lifetime).level;if(a>b){addHearts(1);addMemory('star',`Lila grew to level ${a}!`);setTimeout(()=>{toast(`Lila grew to level ${a}! +1 heart`,'star');confetti();},700);}}
function addHearts(n){state.hearts+=n;}
function addBond(n){const b=bondInfo(state.bond.xp).level;state.bond.xp+=n;const a=bondInfo(state.bond.xp).level;if(a>b){addHearts(1);addMemory('heart',`Your friendship with Lila reached level ${a}`);setTimeout(()=>{toast(`Friendship level ${a}! +1 heart`,'heart');confetti();},600);}}
function addMemory(ic,text){state.journal.unshift({ic,text,date:today()});if(state.journal.length>80)state.journal.pop();}
function registerWin(d){const s=state.streak;if(s.lastWin===d)return;if(!s.lastWin)s.count=1;else{const g=daysBetween(s.lastWin,d);if(g===1)s.count++;else if(g>1){const m=g-1;if(s.freezes>=m){s.freezes-=m;s.count++;toast('Lila kept your streak cozy','flame');}else s.count=1;}}s.lastWin=d;const wasBest=s.best;s.count>s.best&&(s.best=s.count);if(s.best>wasBest&&STREAK_HEART_MILES.includes(s.best)){addHearts(2);addMemory('flame',`${s.best}-day streak! Amazing.`);setTimeout(()=>toast(`${s.best}-day streak! +2 hearts`,'flame'),900);}if(s.count>0&&s.count%5===0)s.freezes=Math.min(3,s.freezes+1);}
function recompute(){const t=today(),day=getDay(t),act=activeHabits();const done=act.filter(h=>countFor(t,h.id)>=1).length;if(done>=dailyGoal()&&state.streak.lastWin!==t)registerWin(t);if(act.length>0&&done===act.length&&!day.perfect){day.perfect=true;addSparks(PERFECT_BONUS);addHearts(1);addBond(4);addMemory('sparkle','A perfect day — every habit done!');setTimeout(()=>{toast(`Perfect day! +${PERFECT_BONUS} sparks +1 heart`,'sparkle');confetti();},250);}save();}
function tapHabit(h){const t=today(),day=getDay(t),cur=countFor(t,h.id);if(h.repeatable){if(cur>=6)return;day.counts[h.id]=cur+1;addSparks(h.sparks);}else{if(cur>=1){day.counts[h.id]=0;state.sparks=Math.max(0,state.sparks-h.sparks);state.lifetime=Math.max(0,state.lifetime-h.sparks);}else{day.counts[h.id]=1;addSparks(h.sparks);if(state.journal.length===0)addMemory('paw','Your first habit with Lila 🐾');}}buzz();celebratePop();recompute();render();}
...
/* ===== care actions ===== */
function petLila(){const day=getDay(today());const n=day.flags.pet||0;if(n>=5){toast('Lila feels so loved today 🤍','heart');return;}day.flags.pet=n+1;addBond(2);buzz();heartBurst();celebratePop();save();render();}
function giveTreat(it){if(!spend(it.cost))return;addBond(it.bond);const firstTime=!state.treatsGiven.includes(it.id);if(firstTime){state.treatsGiven.push(it.id);addMemory('gift',`Shared a ${it.name.toLowerCase()} with Lila`);checkSet('treats');}toast(`Lila loved the ${it.name.toLowerCase()}! +${it.bond} bond`,'heart');confetti();save();render();}

/* ===== buying ===== */
function spend(c){if(state.sparks<c){toast('Keep going — almost there!','sparkle');return false;}state.sparks-=c;return true;}
function spendHearts(c){if(state.hearts<c){toast(`Earn ${c-state.hearts} more hearts for this`,'heart');return false;}state.hearts-=c;return true;}
function checkSet(key){if(setDone(key,state)){addHearts(3);addMemory('trophy',`Completed the ${SETS[key].name} collection! +3 hearts`);setTimeout(()=>{toast(`${SETS[key].name} complete! +3 hearts`,'trophy');confetti();},500);}}
function buyTheme(id){if(state.ownedThemes.includes(id)){state.theme=id;save();render();return;}const t=THEMES.find(x=>x.id===id);if(!spend(t.cost))return;state.ownedThemes.push(id);state.theme=id;toast(`${t.name} room unlocked!`,'gift');confetti();save();render();}
function buyHome(id){if(state.inventory.includes(id))return;const it=DECOR.find(d=>d.id===id);if(!spend(it.cost))return;state.inventory.push(id);addMemory('home',`Added a ${it.name.toLowerCase()} to Lila's home`);toast(`${it.name} added!`,'gift');confetti();checkSet('home');save();render();}
function buyAcc(id){if(state.ownedAcc.includes(id)){state.accessory=state.accessory===id?null:id;save();render();return;}const it=ACC.find(a=>a.id===id);if(!spend(it.cost))return;state.ownedAcc.push(id);state.accessory=id;addBond(3);addMemory('sparkle',`Lila got a new look: ${it.name.toLowerCase()}`);toast(`Lila loves her ${it.name.toLowerCase()}!`,'gift');confetti();checkSet('wardrobe');save();render();}
function unlockPlace(id,cost){if(!spendHearts(cost))return;state.places.push(id);addMemory('sparkles2','Unlocked the Pet Spa in town!');toast('Pet Spa unlocked!','sparkles2');confetti();state.location=id;save();render();}

/* ===== garden ===== */
function plotStage(p){if(!p)return -1;const age=daysBetween(p.planted,today());return age<=0?0:age===1?1:age===2?2:3;}
function plantSeed(idx,seedId){const s=SEEDS.find(x=>x.id===seedId);if(!spend(s.cost))return;state.garden[idx]={seed:seedId,planted:today(),watered:today()};addMemory('sprout',`Planted a ${s.name.toLowerCase()} in the garden`);toast(`${s.name} planted! Come back daily to watch it grow`,'sprout');closeSheet();save();render();}
function water(idx){const p=state.garden[idx];if(!p)return;if(p.watered===today()){toast('Already watered today 🤍','droplet');return;}p.watered=today();addBond(1);addSparks(2);buzz();toast('Watered! +1 bond','droplet');save();render();}
function harvest(idx){const p=state.garden[idx];if(plotStage(p)<3)return;const s=SEEDS.find(x=>x.id===p.seed);addHearts(2);if(!state.gardenHarvested.includes(p.seed)){state.gardenHarvested.push(p.seed);addMemory('sprout',`Your ${s.name.toLowerCase()} bloomed! +2 hearts`);checkSet('garden');}else addMemory('sprout',`Harvested a ${s.name.toLowerCase()} +2 hearts`);state.garden[idx]=null;toast(`${s.name} bloomed! +2 hearts`,'heart');confetti();save();render();}
...
function header(){return`<div class="topbar"><div class="greet"><div class="hi">Hi there</div><div class="nm">${esc(state.name)}</div></div>
...
function viewToday(){const li=levelInfo(state.lifetime),bi=bondInfo(state.bond.xp),act=activeHabits();
  const done=act.filter(h=>countFor(today(),h.id)>=1).length;const perfect=isPerfectToday(),pose=perfect?'celebrate':'hero';
  ...
  let banner;if(perfect)banner=`<div class="banner perfect">${icon('sparkle')}<div>Perfect day — every habit done!<small>Lila is so proud of you</small></div></div>`;
  else{const nx=cheapestLocked();banner=nx?`<div class="banner next">${icon('gift')}<div>Next for Lila: ${esc(nx.name)}<small>${Math.max(0,nx.cost-state.sparks)} more sparks to go</small></div></div>`:`<div class="banner next">${icon('star')}<div>You've unlocked so much!<small>Lila's world is amazing</small></div></div>`;}
  return header()+`
    <div class="lila-card">
      <div class="speech">${esc(note)}</div>
      <div class="lila-stage" data-act="pet" title="pet Lila">${lilaFig(pose,196)}</div>
      <div class="lila-name"><b>Lila</b><span class="lvl">Lv ${li.level}</span><div class="xpbar"><i style="width:${li.pct}%"></i></div></div>
      <div class="bondrow">${icon('heart')}<div class="bondbar"><i style="width:${bi.pct}%"></i></div><span>Friendship ${bi.level}</span></div>
      <div class="pethint">tap Lila to give her a cuddle 🤍</div>
    </div>
...
/* ----- World ----- */
function viewWorld(){switch(state.location){case'home':return worldHome();case'boutique':return shopAcc('Boutique','boutique');case'furniture':return furnitureScreen();case'cafe':return cafeScreen();case'park':return worldPark();case'spa':return spaScreen();default:return worldMap();}}
...
function worldMap(){const spaUnlocked=state.places.includes('spa');
  return header()+`<div class="sec"><h2>Lila's world</h2><span class="meta">tap a place to visit</span></div>
    <div class="worldwrap"><div class="worldmap"><img src="assets/lila/world.png" alt="Lila's town">
    ...
    <div class="newplace ${spaUnlocked?'done':''}" data-act="${spaUnlocked?'goto':'unlockspa'}" data-id="spa">
      ...
      <div class="np-tx"><b>Pet Spa</b><small>${spaUnlocked?'Open — visit for a spa day':`A brand-new place — unlock for ${SPA_COST_HEARTS} hearts`}</small></div>
      <div class="np-cost">${spaUnlocked?icon('back','style="transform:rotate(180deg)"'):icon('heart')+SPA_COST_HEARTS}</div>
    </div>
    <div class="hintline">${icon('sparkle')} <b>${state.sparks}</b> sparks &nbsp;·&nbsp; ${icon('heart')} <b>${state.hearts}</b> hearts</div>`;}
function locHeader(title){return`<div class="locbar"><button class="back" data-act="backmap">${icon('back')} Town</button><div class="chips"><div class="chip heart">${icon('heart')}${state.hearts}</div><div class="chip spark">${icon('sparkle')}${state.sparks}</div></div></div><div class="sec"><h2>${title}</h2></div>`;}
...
function shopAcc(title,shop){const list=ACC.filter(a=>a.shop===shop);
  const cards=list.map(it=>{const owned=state.ownedAcc.includes(it.id),worn=state.accessory===it.id;const cls=owned?'owned':(state.sparks<it.cost?'cant':'');return`<div class="shopcard"><div class="pre">${it.svg.replace('width="100%"','width="62"')}</div><b>${esc(it.name)}</b><button class="buy ${cls}" data-act="buyacc" data-id="${it.id}">${owned?(worn?'Wearing':'Wear'):icon('sparkle')+' '+it.cost}</button></div>`;}).join('');
...
function furnitureScreen(){const themes=THEMES.map(t=>{const owned=state.ownedThemes.includes(t.id),active=state.theme===t.id;return`<div class="shopcard"><div class="pre"><div class="swatch" style="background:${t.wall}"></div></div><b>${t.name}</b><button class="buy ${owned?'owned':(state.sparks<t.cost?'cant':'')}" data-act="buytheme" data-id="${t.id}">${active?'Active':owned?'Equip':icon('sparkle')+' '+t.cost}</button></div>`;}).join('');
  const furn=DECOR.map(d=>{const owned=state.inventory.includes(d.id);return`<div class="shopcard"><div class="pre">${d.svg.replace('width="100%"','width="62"')}</div><b>${esc(d.name)}</b><button class="buy ${owned?'owned':(state.sparks<d.cost?'cant':'')}" data-act="buyhome" data-id="${d.id}">${owned?'Owned':icon('sparkle')+' '+d.cost}</button></div>`;}).join('');
  return locHeader('Furniture shop')+setBar('home')+`<div class="subsec">Room themes</div><div class="shop">${themes}</div><div class="subsec">Furniture</div><div class="shop">${furn}</div><div style="height:8px"></div>`;}
function cafeScreen(){const cards=TREATS.map(it=>{const got=state.treatsGiven.includes(it.id);return`<div class="shopcard"><div class="pre">${it.svg.replace('width="100%"','width="56"')}</div><b>${esc(it.name)} ${got?icon('check','style="width:13px;height:13px;color:#2F9B72;vertical-align:-1px"'):''}</b><button class="buy ${state.sparks<it.cost?'cant':''}" data-act="treat" data-id="${it.id}">${icon('heart')} +${it.bond} · ${icon('sparkle')}${it.cost}</button></div>`;}).join('');
  return locHeader('Café')+`<div class="shoplila">${lilaFig('hero',150)}</div><p class="note">Treat Lila to something yummy — it deepens your friendship and fills her collection.</p>${setBar('treats')}<div class="shop">${cards}</div><div style="height:8px"></div>`;}
function worldHome(){const th=THEMES.find(t=>t.id===state.theme)||THEMES[0];
  ...
  const wardrobe=state.ownedAcc.length?`<div class="subsec">Lila's wardrobe</div><div class="wardrobe"><button class="wbtn ${!state.accessory?'sel':''}" data-act="equipacc" data-id="">${icon('paw')}</button>${state.ownedAcc.map(id=>{const a=ACC.find(x=>x.id===id);return`<button class="wbtn ${state.accessory===id?'sel':''}" data-act="equipacc" data-id="${id}">${a.svg.replace('width="100%"','width="34"')}</button>`;}).join('')}</div>`:'';
  return locHeader("Lila's home")+`<div class="room-wrap"><div class="room care-stage" data-act="pet"><div style="position:absolute;inset:0;background:${th.wall}"></div><div class="floor" style="background:${th.floor}"></div>${items}<div class="roomlila">${lilaFig('hero',176)}</div></div></div>${wardrobe}<div class="hintline">${icon('home')} Visit the shops in town to make Lila's home cozier</div><div style="height:8px"></div>`;}
function worldPark(){const walk=state.habits.find(h=>h.id==='walk');
  ...
  const plots=state.garden.map((p,i)=>{
    if(!p)return`<div class="plot empty" data-act="plant" data-id="${i}"><div class="soil">${icon('plus')}</div><small>Plant</small></div>`;
    ...
    const action=st>=3?`<button class="pbtn harvest" data-act="harvest" data-id="${i}">Harvest ${icon('heart')}</button>`:`<button class="pbtn ${p.watered===today()?'wet':''}" data-act="water" data-id="${i}">${p.watered===today()?'Watered':'Water'}</button>`;
    return`<div class="plot"><div class="soil grown">${art}</div><small>${st>=3?s.name:['Seed','Sprout','Bud'][st]}</small>${action}</div>`;}).join('');
  ...
  return locHeader('Park & garden')+`<div class="room-wrap"><div class="room park"><div style="position:absolute;inset:0;background:linear-gradient(180deg,#CFEFDD,#BCE6CF)"></div><div class="floor" style="background:#A9DCC0"></div>${blooms}<div class="roomlila">${lilaFig('hero',146)}</div></div></div>
    <button class="walkbtn" data-act="walk">${icon('walk')} Take Lila for a walk <span>+${walk?walk.sparks:12} ✦</span></button>
    <div class="subsec">Your garden</div><div class="garden">${plots}</div>
    ${setBar('garden')}<div style="height:6px"></div>`;}
function spaScreen(){const day=getDay(today());const spaDone=day.flags.spa;const list=ACC.filter(a=>a.shop==='spa');
  const cards=list.map(it=>{const owned=state.ownedAcc.includes(it.id),worn=state.accessory===it.id;return`<div class="shopcard"><div class="pre">${it.svg.replace('width="100%"','width="62"')}</div><b>${esc(it.name)}</b><button class="buy ${owned?'owned':(state.sparks<it.cost?'cant':'')}" data-act="buyacc" data-id="${it.id}">${owned?(worn?'Wearing':'Wear'):icon('sparkle')+' '+it.cost}</button></div>`;}).join('');
  return locHeader('Pet Spa')+`<div class="room-wrap"><div class="room care-stage" data-act="pet"><div style="position:absolute;inset:0;background:linear-gradient(180deg,#E3F1FB,#E9E4F7)"></div><div class="floor" style="background:#D4E6F2"></div><div class="roomlila">${lilaFig('hero',160)}</div></div></div>
    <button class="walkbtn" style="background:#9C86D6" data-act="spaday" ${spaDone?'disabled style="opacity:.5;background:#9C86D6"':''}>${icon('sparkles2')} ${spaDone?'Lila feels fabulous today':'Give Lila a spa day'} <span>+6 ♥</span></button>
    <div class="subsec">Spa boutique</div><div class="shop">${cards}</div><div style="height:8px"></div>`;}

/* ----- Journey (stats + memories + collections) ----- */
function viewJourney(){const names=['S','M','T','W','T','F','S'];let cells='';
  ...
  const mems=state.journal.length?state.journal.slice(0,18).map(m=>`<div class="mem"><div class="mem-ic">${icon(m.ic)}</div><div class="mem-tx"><b>${esc(m.text)}</b><small>${relDay(m.date)}</small></div></div>`).join(''):`<p class="note">Your memories with Lila will appear here as you go 🤍</p>`;
  ...
  return header()+`<div class="sec"><h2>This week</h2></div><div class="cal">${cells}</div>
    <div class="stats"><div class="stat"><div class="n">${displayStreak()}</div><div class="l">Day streak${state.streak.freezes?` · ${state.streak.freezes} ❄`:''}</div></div><div class="stat"><div class="n">${bi.level}</div><div class="l">Friendship level</div></div><div class="stat"><div class="n">${state.lifetime}</div><div class="l">Sparks earned</div></div><div class="stat"><div class="n">${state.hearts}</div><div class="l">Hearts</div></div></div>
    <div class="sec"><h2>Memories</h2><span class="meta">${icon('book')}</span></div><div class="memwrap">${mems}</div>
    <div class="sec"><h2>Collections</h2></div><div class="cols">${cols}</div>
    <div class="sec"><h2>Badges</h2><span class="meta">Lila lv ${li.level}</span></div><div class="badges">${badges}</div><div style="height:8px"></div>`;}

/* ----- Habits ----- */
function viewHabits(){const rows=state.habits.map(h=>{const t=TINT[h.color]||TINT.pink;return`<div class="erow ${h.active?'':'off'}"><div class="ic" style="width:38px;height:38px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:${t.bg};color:${t.fg}">${icon(h.icon)}</div><div class="tx" ${h.builtin?'':`data-act="edit" data-id="${h.id}"`}><b>${esc(h.name)} ${h.builtin?'':icon('pencil','style="width:13px;height:13px;vertical-align:-1px;opacity:.5"')}</b><span>${esc(h.sub)} · ${h.sparks} sparks${h.repeatable?' · repeatable':''}</span></div><div class="toggle ${h.active?'on':''}" data-act="toggle" data-id="${h.id}"><i></i></div></div>`;}).join('');
  return header()+`<div class="sec"><h2>Your habits</h2><span class="meta">${activeHabits().length} active</span></div><p class="note">Turn habits on or off any time. You can always add your own too.</p><div class="edit">${rows}</div><button class="addbtn" data-act="add">${icon('plus')} Add my own habit</button><div style="height:8px"></div>`;}

/* ===== sheets ===== */
let sheetEditId=null,sheetIcon='star';
function openSheet(id){sheetEditId=id;const h=id?state.habits.find(x=>x.id===id):null;sheetIcon=h?h.icon:'star';const picks=['star','heart','leaf','droplet','sun','moon','barbell','walk','apple','bolt','shoe','sparkle'];const scrim=document.getElementById('scrim');scrim.innerHTML=`<div class="sheet"><h3>${h?'Edit habit':'New habit'}</h3><div class="field"><label>Name</label><input id="f-name" maxlength="28" placeholder="e.g. Stretch break" value="${h?esc(h.name):''}"></div><div class="field"><label>Little note</label><input id="f-sub" maxlength="40" placeholder="e.g. 5 minutes of stretching" value="${h?esc(h.sub):''}"></div><div class="field"><label>Pick an icon</label><div class="iconpick" id="f-icons">${picks.map(p=>`<button data-act="picon" data-id="${p}" class="${p===sheetIcon?'sel':''}">${icon(p)}</button>`).join('')}</div></div><div class="row">${h&&!h.builtin?`<button class="cancel" data-act="delhabit" data-id="${id}" style="flex:.6;color:var(--pink-deep)">Delete</button>`:''}<button class="cancel" data-act="closesheet">Cancel</button><button class="save" data-act="savehabit">Save</button></div></div>`;scrim.classList.add('show');}
function openSeedSheet(idx){const scrim=document.getElementById('scrim');scrim.innerHTML=`<div class="sheet"><h3>Plant a seed</h3><p class="note" style="margin:0 0 10px">It grows over a few real days — water it daily and watch it bloom.</p><div class="shop">${SEEDS.map(s=>`<div class="shopcard"><div class="pre">${s.bloom.replace('width="100%"','width="50"')}</div><b>${esc(s.name)}</b><button class="buy ${state.sparks<s.cost?'cant':''}" data-act="doplant" data-id="${idx}:${s.id}">${icon('sparkle')} ${s.cost}</button></div>`).join('')}</div><div class="row"><button class="cancel" data-act="closesheet">Cancel</button></div></div>`;scrim.classList.add('show');}
function closeSheet(){const s=document.getElementById('scrim');s.classList.remove('show');setTimeout(()=>s.innerHTML='',250);}
function saveHabit(){const name=document.getElementById('f-name').value.trim()||'My habit';const sub=document.getElementById('f-sub').value.trim()||'A healthy little win';if(sheetEditId){const h=state.habits.find(x=>x.id===sheetEditId);h.name=name;h.sub=sub;h.icon=sheetIcon;}else state.habits.push({id:'c'+Date.now(),name,sub,icon:sheetIcon,color:'pink',sparks:10,repeatable:false,active:true,builtin:false});closeSheet();save();render();}
...
function handle(a,id){switch(a){
  ...
  case'spaday':{const day=getDay(today());if(day.flags.spa){toast('Already pampered today 🤍','heart');break;}day.flags.spa=1;addBond(6);addMemory('sparkles2','Spa day with Lila — so relaxing');toast('Lila feels fabulous! +6 bond','sparkles2');confetti();save();render();break;}
  ...
  case'walk':{const w=state.habits.find(h=>h.id==='walk');if(w&&w.active){tapHabit(w);addBond(3);}else toast('Turn on "Move after a meal" in Habits','heart');break;}
...
// daily visit bond
if(state.lastVisit!==today()){const gap=state.lastVisit?daysBetween(state.lastVisit,today()):0;addBond(5);if(gap>1)setTimeout(()=>toast('Lila missed you! Welcome back 🤍','heart'),500);state.lastVisit=today();save();}
render();
